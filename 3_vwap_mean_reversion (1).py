...
# ══════════════════════════════════════════════════════════════
# Extreme 3-VWAP Mean Reversion — Python backtest (Colab-ready)
# Runs on BOTH ES and NQ in a single run. Copy-paste all into one cell.
# ══════════════════════════════════════════════════════════════

# On Colab, run this first in a separate cell: !pip install kagglehub -q
...
# ══════════════════════════════════════════════════════════════
# 0. DOWNLOAD DATASET (once, shared by both instruments)
# ══════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════
# STRATEGY PARAMS (same defaults as the Pine script — shared by both)
# ══════════════════════════════════════════════════════════════
BAND          = 0.00616
ADX_LEN       = 14
ADX_THRESHOLD = 20.93
...
def run_backtest(df):
    cum_return = 1.0   # tracked as a multiplier, no starting capital needed
    position = 0
    entry_price = 0.0
    entry_i = 0
    trades = []
...
for hint, label in INSTRUMENTS.items():
    ...
    if not matches:
        print(f"\n⚠️  No file found for {hint} — skipping.")
        continue
    csv_path = matches[0]

    print("\n" + "█" * 60)
    print(f"  {hint} ({label})  —  {os.path.basename(csv_path)}")
    print("█" * 60)
    ...
    # keep a copy of this instrument's df/trades so we can independently
    # verify the numbers after the loop finishes (see sanity check below)
    if hint == "ES":
...
# ══════════════════════════════════════════════════════════════
print("\n" + "═" * 60)
print("ES vs NQ COMPARISON")
print("═" * 60)
print(pd.DataFrame(summary_rows).to_string(index=False))

# ══════════════════════════════════════════════════════════════
# SANITY CHECK — independent verification that the 22/22 match is real,
# not a bug (confirms signal counts differ before filtering to trades)
# ══════════════════════════════════════════════════════════════
print("\n" + "═" * 60)
print("SANITY CHECK")
print("═" * 60)
